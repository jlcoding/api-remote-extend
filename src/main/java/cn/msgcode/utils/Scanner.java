...
public class Scanner {
        ...
        try {
            ...
            while (dirs.hasMoreElements()){
                ...
                if ("file".equals(protocol)) {
                    ...
                    String filePath = URLDecoder.decode(url.getFile(), "UTF-8");
                    //以文件的方式扫描整个包下的文件 并添加到集合中
                    findAllFile(dir, filePath, recursive, files, suffix);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
     ...
     * 获取匹配类型的文件
     */
    public static void findAllFile(String directory, String directoryPath, final boolean recursive, List<File> files, String suffix){
        //获取此包的目录 建立一个File
        ...
        for (File file : dirfiles) {
            //如果是目录 则继续扫描
            if (file.isDirectory()) {
                findAllFile(directory + "." + file.getName(),
